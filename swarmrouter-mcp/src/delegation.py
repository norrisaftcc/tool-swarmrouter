...
class TaskDelegator:
    ...
    def __init__(self):
        ...
        self.dance_keywords = {
            DanceType.WAGGLE: [
                "complex", "decompose", "analyze", "multi-step", "elaborate",
                "comprehensive", "detailed", "break down", "architect"
            ],
            DanceType.ROUND: [
                "simple", "notify", "alert", "inform", "quick", "brief",
                "announcement", "update", "status"
            ],
            DanceType.SCOUT: [
                "research", "explore", "find", "discover", "investigate",
                "search", "locate", "identify", "survey"
            ],
            DanceType.TREMBLE: [
                "error", "issue", "problem", "fix", "debug", "troubleshoot",
                "resolve", "broken", "failed"
            ],
            DanceType.CONVERGE: [
                "consensus", "agree", "decide", "vote", "collaborate",
                "merge", "combine", "unify", "coordinate"
            ],
            DanceType.DISPERSE: [
                "parallel", "distribute", "split", "concurrent", "multiple",
                "simultaneous", "spread", "divide", "batch"
            ]
        }
    ...
    def determine_dance_type(self, task_description: str) -> DanceType:
        ...
        if scores[best_dance] == 0:
            logger.info(f"No keywords matched, defaulting to WAGGLE dance")
            return DanceType.WAGGLE
            
        logger.info(f"Selected {best_dance} dance based on task description")
        return best_dance
    
    def estimate_tokens_per_bee(self, 
                               total_tokens: int, 
                               num_subtasks: int,
                               dance_type: DanceType) -> int:
        """
    ...
    def create_bees_for_task(self, 
                            task: SwarmTask,
                            subtasks: Optional[List[str]] = None) -> List[BeeMetadata]:
        """
        ...
        if not subtasks:
            ...
            subtasks = self.generate_default_subtasks(
                task.description, 
                task.dance_type
            )
    ...
    def generate_default_subtasks(self, 
                                 description: str,
                                 dance_type: DanceType) -> List[str]:
        """
        Generate default subtasks based on dance type.
        ...
        subtask_templates = {
            ...
            DanceType.SCOUT: [
                ...
                f"Identify best practices",
                f"Compile findings report"
            ],
            DanceType.TREMBLE: [
                f"Identify root cause of: {description}",
                f"Develop fix or workaround",
                f"Test and validate solution"
            ],
            DanceType.CONVERGE: [
                f"Gather perspectives on: {description}",
                f"Synthesize different viewpoints",
