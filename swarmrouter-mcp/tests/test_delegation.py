...
import pytest
from src.models import DanceType, TaskPriority, TaskStatus, TaskDelegationRequest
...
class TestTaskDelegator:
    ...
    def test_generate_default_subtasks(self, delegator):
        ...
        subtasks = delegator.generate_default_subtasks(description, DanceType.WAGGLE)
        assert len(subtasks) == 4
        assert "Analyze requirements" in subtasks[0]
        assert "Design solution" in subtasks[1]

        # Round dance should generate 1 subtask
        subtasks = delegator.generate_default_subtasks(description, DanceType.ROUND)
        assert len(subtasks) == 1
        assert subtasks[0] == f"Execute: {description}"

        # Scout dance should generate 3 subtasks
        subtasks = delegator.generate_default_subtasks(description, DanceType.SCOUT)
    ...
    def test_create_bees_for_task(self, delegator):
        ...
        expected_tokens = int(
            (10000 // 3) * 0.3
        )  # Account for integer division in implementation
    ...
    def test_get_swarm_statistics_with_tasks(self, delegator):
        ...
        # Create some tasks
        for i in range(3):
            request = TaskDelegationRequest(description=f"Task {i}", max_tokens=10000)
            task = delegator.delegate_task(request)
