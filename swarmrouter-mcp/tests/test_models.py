...
class TestBeeMetadata:
    ...
    def test_bee_efficiency_calculation(self):
        ...
        bee = BeeMetadata(dance_type=DanceType.WAGGLE, estimated_tokens=1000)
...
class TestSwarmTask:
    ...
    def test_assign_bee(self):
        ...
        task = SwarmTask(description="Test task", dance_type=DanceType.ROUND)
    ...
    def test_token_savings_calculation(self):
        ...
        task = SwarmTask(
            description="Complex task", dance_type=DanceType.WAGGLE, max_tokens=10000
        )

        # Add bees with actual token usage
        bee1 = BeeMetadata(
            dance_type=DanceType.WAGGLE, estimated_tokens=3000, actual_tokens=2000
        )
        bee2 = BeeMetadata(
            dance_type=DanceType.WAGGLE, estimated_tokens=3000, actual_tokens=1500
        )
        ...
        bee3 = BeeMetadata(dance_type=DanceType.WAGGLE, estimated_tokens=2000)
...
class TestModelValidation:
    ...
    def test_bee_metadata_validation(self):
        ...
        # Valid bee
        bee = BeeMetadata(dance_type=DanceType.WAGGLE, estimated_tokens=100)
        assert bee.estimated_tokens > 0

        # Invalid: negative tokens
        with pytest.raises(ValueError):
            BeeMetadata(dance_type=DanceType.WAGGLE, estimated_tokens=-100)

    def test_swarm_task_validation(self):
        """Test SwarmTask validation."""
        # Valid task
        task = SwarmTask(
            description="Valid task", dance_type=DanceType.ROUND, max_tokens=1000
        )
        ...
        with pytest.raises(ValueError):
            SwarmTask(
                description="Invalid task", dance_type=DanceType.ROUND, max_tokens=-1000
            )
