...
class SwarmCoordinator:
    ...
    async def execute_with_swarm(self, task: str) -> Dict:
        ...
        # Create worker bees
        worker_bees = [
            WorkerBee(f"bee_{i+1}") for i in range(len(decomposition["subtasks"]))
        ]

        # Execute based on strategy
        if decomposition["coordination_strategy"] == "parallel":
            results = await self._execute_parallel(
                worker_bees, decomposition["subtasks"]
            )
            ...
            results = await self._execute_converge(
                worker_bees, decomposition["subtasks"]
            )
        ...
        savings_percentage = (
            (estimated_direct_tokens - total_tokens) / estimated_direct_tokens
        ) * 100
    ...
    async def _execute_parallel(
        self, bees: List[WorkerBee], subtasks: List[Dict]
    ) -> List[Dict]:
        """Execute subtasks in parallel."""
        print("\n🔄 Executing subtasks in parallel...")

        tasks = [bee.execute_task(subtask) for bee, subtask in zip(bees, subtasks)]

        results = await asyncio.gather(*tasks)

        for result in results:
            print(
                f"  ✅ {result['bee_id']} completed (tokens: {result['tokens_used']})"
            )
...
async def demonstrate_swarm_patterns():
    ...
    for example in examples:
        ...
        print(
            f"  Estimated Direct: {result['token_usage']['estimated_direct_tokens']} tokens"
        )
        ...
        for i, r in enumerate(result["results"][:3]):  # Show first 3 results
            if "result" in r:
                preview = (
                    r["result"][:100] + "..." if len(r["result"]) > 100 else r["result"]
                )
